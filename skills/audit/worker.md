# /audit Worker — Comprehensive Codebase Audit
...
- **MCP server checks**: If the project is an MCP server (look for MCP dependencies in the manifest — e.g., `rmcp` in Cargo.toml, `@modelcontextprotocol/sdk` in package.json — or a `serve` subcommand, or "MCP" in CLAUDE.md/description), check:
  - **Installation instructions in agents-guide**: The `agents-guide.md` must include a complete Installation section with: (1) binary install command (`brew install ...` or equivalent), (2) MCP server registration for Claude Code specifically (`claude mcp add --scope user <name> -- <binary> serve`), (3) generic MCP client JSON config block, (4) explicit note that the agent session must be restarted after registration. Flag any missing steps — agents that lack these instructions will improvise incorrect installation paths.
  - **Installation instructions in README**: The README must have a corresponding Installation section covering the same steps, plus explicit agent-facing instructions in a "For AI agents" section that tells an agent exactly what commands to run if asked to install the server.
  - **Server instructions**: The MCP server's `get_info()` (or equivalent initialisation handshake) should serve substantive instructions — at minimum the agents-guide content, not just a brief tool list. Check for `with_instructions` or the framework's equivalent and verify the content is non-trivial.
  - **`get_agent_prompt` tool** (or equivalent): For servers that store project-specific state (recipes, conventions, learned patterns), check for a tool that returns dynamic context (the static guide plus project-specific state). Not all servers need this — only those with persistent learned state.
