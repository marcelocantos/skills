...
10. **Install locally**: Install the released version onto the laptop so the user can use it immediately. This step is **mandatory** for projects with a Homebrew tap — do not skip it and do not ask for permission.
   ...
   **Brew-managed tap merge conflicts.** When `brew update` fetches the tap and your local copy at `/opt/homebrew/Library/Taps/marcelocantos/homebrew-tap` has diverged from `origin/master`, the resulting merge conflict surfaces as a Ruby parse error during the next `brew upgrade` (e.g. `unexpected '>>>>>>> Stashed changes'`) and the upgrade aborts before installing the new version — `<project> --version` then still reports the old version and looks like the release didn't take. Diagnose with `git -C /opt/homebrew/Library/Taps/marcelocantos/homebrew-tap status -sb`; resolve with `git -C ... checkout --theirs Formula/<project>.rb && git -C ... checkout -- Formula/<project>.rb` to take the homebrew-releaser-generated version, then re-run `brew upgrade`. The divergence is almost always caused by *testing-now* hand-edits to the brew-managed tap during the release cycle (e.g. `cp <working-dir>/Formula/<project>.rb /opt/homebrew/Library/Taps/.../Formula/`); the safer pattern for testing a tap-side change pre-release is to push the change to a branch on `origin` and `git -C <brew-managed-tap> fetch && git -C <brew-managed-tap> reset --hard origin/<branch>`, which keeps the brew-managed copy a true mirror with no working-tree edits to conflict on.
