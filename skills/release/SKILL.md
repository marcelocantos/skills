...
5. **Homebrew tap**: Check if `marcelocantos/homebrew-tap` exists and whether it already has a formula for this project. Also check that the **`HOMEBREW_TAP_TOKEN` action secret is set on this repo** — `discover.sh` reports this as `homebrew_tap_token_secret` (`set` / `missing`). homebrew-releaser reads this secret to push the generated formula into the tap; when it's missing, the job fails with the unhelpful error *"You must provide all necessary environment variables."* on the first release. First-release-of-a-new-repo is the common case — resolve it now from 1Password (see Phase 4 step 2 for the `op read` + `gh secret set` commands) rather than discovering it post-tag and having to re-run the failed homebrew-releaser job by hand.
   ...
   **A `set` secret is not a *valid* secret.** Action secrets are write-only, so `homebrew_tap_token_secret: set` only proves the secret *exists* — not that its value still authenticates. A PAT that expired since it was stored will pass this check and then 401 in the homebrew-releaser job after the tag is cut. `discover.sh` therefore also probes the 1Password *source* token and reports `homebrew_tap_token_op` (`valid` / `expired` / unavailable). Read it: if `expired`, the PAT itself has lapsed and must be re-minted before tagging (see Phase 4 step 2); if `valid`, Phase 4 step 2 refreshes the repo secret unconditionally so a drifted secret value can't 401.
...
2. **Add homebrew-releaser job**: Add a job that runs after binaries are uploaded, using [homebrew-releaser](https://github.com/Justintime50/homebrew-releaser):
   ...
   - **`HOMEBREW_TAP_TOKEN` secret**: A shared PAT for all repos is stored in 1Password (refreshing it fixes every repo's release at once). Set it on a new repo — and **refresh it on every release** (idempotent; eliminates the stale-secret 401 below) whenever `discover.sh`'s `homebrew_tap_token_op` is `valid`:
     ```bash
     op read "op://Personal/GitHub Homebrew Tap PAT/token" | gh secret set HOMEBREW_TAP_TOKEN --repo <owner>/<repo>
     ```
     If `homebrew_tap_token_op` is `expired`, the PAT itself has lapsed — the user must mint a new fine-grained PAT with access to `marcelocantos/homebrew-tap` and update the 1Password item before this will help. (`gh secret set` reads the piped value from stdin, so the token is never echoed.)
   ...
   **Known homebrew-releaser issues:**
   ...
   - **Stale `HOMEBREW_TAP_TOKEN` → opaque `401 Unauthorized`.** homebrew-releaser's first GitHub API call (`GET /repos/<owner>/<repo>`) fails with `requests.exceptions.HTTPError: 401` when the stored secret's PAT has expired — even though the secret exists. The binaries upload before this job, so the release is intact; only the tap formula is unpushed. Recovery: refresh the secret from 1Password (Phase 4 step 2) and re-run just the failed job — `gh run rerun <run-id> --failed` — then re-watch. Catch it pre-tag by reading `homebrew_tap_token_op` (see B.1 step 5).
