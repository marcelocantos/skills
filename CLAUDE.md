...
- **One PR per objective, not per step. HARD RULE.** A PR's unit is a whole feature or objective — *not* a phase, sub-target, or increment of one. This is a **planning-time** rule: if your plan maps phases or sub-targets (🎯T1.1, T1.2, …) to separate PRs, the plan is wrong — collapse them into one branch where phases are *commits*, and open a single PR when the whole objective lands. **Primary reason: velocity.** Every PR boundary is a hard stop waiting on human review and merge; N PRs for one feature means N stalls in your throughput. (Secondary: I'm on free GitHub — no merge queue, so each inflight PR also costs manual rebasing and a separate CI cycle.) Corollaries: never have two PRs open at once; never start a second while one is inflight; bundle related changes into one larger PR *even when each part is independently reviewable* — reviewability is not a reason to split. Split into multiple PRs **only** when parts are genuinely independent *and* separately useful on their own, or when I explicitly tell you to. When unsure, one bigger PR always wins.
